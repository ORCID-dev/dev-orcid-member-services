...
public class AffiliationAdapter {
      ...
      public static Affiliation toOrcidAffiliation(Assertion assertion) {
            ...
            switch (assertion.getAffiliationSection()) {
                  ...
                  default:
                        throw new IllegalArgumentException(
                              "Invalid affiliation section found"
                        );
            }

            if (!StringUtils.isBlank(assertion.getPutCode())) {
                  orcidAffiliation.setPutCode(
                        Long.valueOf(assertion.getPutCode())
                  );
            }
            ...
            if (!StringUtils.isBlank(assertion.getEndYear())) {
                  ...
                  endDate.setYear(
                        new Year(Integer.valueOf(assertion.getEndYear()))
                  );
                  endDate.setMonth(
                        StringUtils.isBlank(assertion.getEndMonth())
                              ? null
                              : new Month(
                                    Integer.valueOf(assertion.getEndMonth())
                              )
                  );
                  endDate.setDay(
                        StringUtils.isBlank(assertion.getEndDay())
                              ? null
                              : new Day(Integer.valueOf(assertion.getEndDay()))
                  );
                  orcidAffiliation.setEndDate(endDate);
            }

            if (
                  !StringUtils.isBlank(assertion.getExternalId()) &&
                  !StringUtils.isBlank(assertion.getExternalIdType())
            ) {
                  ...
                  extId.setUrl(
                        StringUtils.isBlank(assertion.getExternalIdUrl())
                              ? null
                              : new Url(assertion.getExternalIdUrl())
                  );
            ...
            address.setCountry(
                  Iso3166Country.valueOf(assertion.getOrgCountry())
            );
            address.setRegion(
                  StringUtils.isBlank(assertion.getOrgRegion())
                        ? null
                        : assertion.getOrgRegion()
            );
            org.setAddress(address);

            DisambiguatedOrganization disambiguatedOrg =
                  new DisambiguatedOrganization();
            disambiguatedOrg.setDisambiguatedOrganizationIdentifier(
                  assertion.getDisambiguatedOrgId()
            );
            disambiguatedOrg.setDisambiguationSource(
            ...
            if (!StringUtils.isBlank(assertion.getStartYear())) {
                  ...
                  startDate.setYear(
                        new Year(Integer.valueOf(assertion.getStartYear()))
                  );
                  startDate.setMonth(
                        StringUtils.isBlank(assertion.getStartMonth())
                              ? null
                              : new Month(
                                    Integer.valueOf(assertion.getStartMonth())
                              )
                  );
                  startDate.setDay(
                        StringUtils.isBlank(assertion.getStartDay())
                              ? null
                              : new Day(
                                    Integer.valueOf(assertion.getStartDay())
                              )
                  );
                  orcidAffiliation.setStartDate(startDate);
            }

            orcidAffiliation.setUrl(
                  StringUtils.isBlank(assertion.getUrl())
                        ? null
                        : new Url(assertion.getUrl())
            );

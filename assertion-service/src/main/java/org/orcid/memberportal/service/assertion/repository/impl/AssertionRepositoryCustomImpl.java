...
public class AssertionRepositoryCustomImpl implements AssertionRepositoryCustom {
  ...
  public List<Assertion> findAllToUpdateInOrcidRegistry(Pageable pageable) {
    ProjectionOperation timeModifiedAfterSync = Aggregation
      .project("added_to_orcid", "updated_in_orcid", "modified", "created")
  ...
  public List<MemberAssertionStatusCount> getMemberAssertionStatusCounts() {
    ...
    AggregationResults<MemberAssertionStatusCount> results = mongoTemplate.aggregate(
      aggregation,
      "assertion",
  ...
  public Iterator<String> findDistinctEmailsWithNotificationRequested(String salesforceId) {
    DistinctIterable<String> distinctIterable = mongoTemplate
      ...
      .distinct(
        "email",
        Filters.and(Filters.eq("status", AssertionStatus.NOTIFICATION_REQUESTED.name()), Filters.eq("salesforce_id", salesforceId)),
        String.class
      );

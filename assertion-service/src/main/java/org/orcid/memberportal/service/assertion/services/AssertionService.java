...
public class AssertionService {
      ...
      public Page<Assertion> findByOwnerId(Pageable pageable) {
            return assertionRepository.findByOwnerId(
                  assertionsUserService.getLoggedInUserId(),
                  pageable
            );
      }

      public List<Assertion> findAllByOwnerId() {
            return assertionRepository.findAllByOwnerId(
                  assertionsUserService.getLoggedInUserId(),
                  SORT
            );
      }
      ...
      public Page<Assertion> findByCurrentSalesforceId(Pageable pageable) {
            return findBySalesforceId(
                  assertionsUserService.getLoggedInUserSalesforceId(),
                  pageable
            );
      }

      public Page<Assertion> findBySalesforceId(
            Pageable pageable,
            String filter
      ) {
            String salesforceId =
      ...
      public void deleteAllBySalesforceId(String salesforceId) {
            List<Assertion> assertions = assertionRepository.findBySalesforceId(
                  salesforceId,
                  SORT
            );
            assertions.forEach(a -> {
                  ...
                  Optional<OrcidRecord> orcidRecordOptional =
                        orcidRecordService.findOneByEmail(assertionEmail);
                  if (orcidRecordOptional.isPresent()) {
                        orcidRecordService.deleteOrcidRecordByEmail(
                              assertionEmail
                        );
                  }
            });
      ...
      public void populatePermissionLink(Assertion assertion) {
            assertion.setPermissionLink(
                  orcidRecordService.generateLinkForEmail(assertion.getEmail())
            );
      }

      public Assertion createAssertion(
            Assertion assertion,
            AssertionServiceUser owner
      ) {
            ...
            Optional<OrcidRecord> optionalRecord =
                  orcidRecordService.findOneByEmail(email);
            if (!optionalRecord.isPresent()) {
                  orcidRecordService.createOrcidRecord(
                        email,
                        now,
                        assertion.getSalesforceId()
                  );
            } else {
                  OrcidRecord record = optionalRecord.get();
                  if (
                        record.getTokens() == null ||
                        record.getTokens().isEmpty() ||
                        !record.tokenExists(assertion.getSalesforceId())
                  ) {
                        ...
                        AssertionStatus tokenDeniedStatus =
                              checkForTokenDeniedStatus(
                                    optionalRecord,
                                    assertion
                              );
                        if (tokenDeniedStatus == null) {
                              String activeToken = record.getToken(
                                    assertion.getSalesforceId(),
                                    false
                              );
                              if (
      ...
      public Assertion updateAssertion(
            ...
            Optional<Assertion> optional = assertionRepository.findById(
                  assertion.getId()
            );
            ...
            if (
                  !user
                        .getSalesforceId()
                        .equals(existingAssertion.getSalesforceId())
            ) {
                  throw new BadRequestAlertException(
                        "Illegal assertion access",
                        "affiliation",
      ...
      private String getAssertionStatus(Assertion assertion) {
            Optional<OrcidRecord> optionalRecord =
                  orcidRecordService.findOneByEmail(assertion.getEmail());
            AssertionStatus tokenDeniedStatus = checkForTokenDeniedStatus(
                  optionalRecord,
                  assertion
            );
            if (tokenDeniedStatus != null) {
                  ...
                  AssertionStatus.ERROR_ADDING_TO_ORCID
                        .name()
                        .equals(assertion.getStatus()) ||
                  AssertionStatus.ERROR_UPDATING_TO_ORCID
                        .name()
                        .equals(assertion.getStatus())
            ) {
                  return AssertionStatus.PENDING_RETRY.name();
            } else if (
                  AssertionStatus.ERROR_DELETING_IN_ORCID
                        .name()
                        .equals(assertion.getStatus())
            ) {
                  return AssertionStatus.ERROR_DELETING_IN_ORCID.name();
            } else if (
                  AssertionStatus.PENDING.name().equals(assertion.getStatus())
            ) {
                  return AssertionStatus.PENDING.name();
            } else if (
                  AssertionStatus.NOTIFICATION_SENT
                        .name()
                        .equals(assertion.getStatus())
            ) {
                  return AssertionStatus.NOTIFICATION_SENT.name();
            } else if (
                  AssertionStatus.NOTIFICATION_REQUESTED
                        .name()
                        .equals(assertion.getStatus())
            ) {
                  return AssertionStatus.NOTIFICATION_REQUESTED.name();
            } else if (
                  AssertionStatus.NOTIFICATION_FAILED
                        .name()
                        .equals(assertion.getStatus())
            ) {
      ...
      private boolean updateAssertionsSalesforceId(
            String from,
            String to,
            boolean rollback
      ) {
            try {
                  ...
                  Page<Assertion> page = assertionRepository.findBySalesforceId(
                        from,
                        pageable
                  );
                  while (!page.isEmpty()) {
                  ...
                  LOG.error(
                        "Error bulk updating assertions from salesforce '" +
                        from +
                        "' to salesforce '" +
                        to +
                        "'",
                  ...
                  if (rollback) {
                        LOG.info(
                              "Attempting to RESET assertion salesforce ids from '{}' to '{}'",
                              new Object[] { to, from }
                        );
                        boolean success = updateAssertionsSalesforceId(
                              to,
                              from,
                              false
                        );
                        if (success) {
                              LOG.info(
                                    "Succeeded in RESETTING assertion salesforce ids from '{}' to '{}'",
                                    new Object[] { to, from }
                              );
                              return false;
                        } else {
                              LOG.error(
                                    "Failed to reset assertions from '{}' to '{}'",
                                    new Object[] { to, from }
                              );
                              LOG.error(
      ...
      public void deleteById(String id, AssertionServiceUser user)
            throws RegistryDeleteFailureException {
      ...
      private boolean duplicates(Assertion a, Assertion b) {
            ...
            return (
                  !different(
                        a.getAffiliationSection(),
                        b.getAffiliationSection()
                  ) &&
                  ...
                  !different(
                        a.getDisambiguationSource(),
                        b.getDisambiguationSource()
                  ) &&
                  !different(
                        a.getDisambiguatedOrgId(),
                        b.getDisambiguatedOrgId()
                  ) &&
      ...
      private boolean different(
            AffiliationSection affiliationSectionA,
            AffiliationSection affiliationSectionB
      ) {
      ...
      private boolean different(String fieldA, String fieldB) {
            if (
                  (fieldA == null || fieldA.isEmpty()) &&
                  (fieldB == null || fieldB.isEmpty())
            ) {
                  return false;
            }
      ...
      private boolean assertionToDelete(Assertion assertion) {
            return (
                  assertion.getId() != null &&
                  assertion.getAddedToORCID() == null &&
                  assertion.getAffiliationSection() == null &&
                  assertion.getCreated() == null &&
                  assertion.getDepartmentName() == null &&
                  assertion.getDisambiguatedOrgId() == null &&
                  assertion.getDisambiguationSource() == null &&
                  assertion.getEmail() == null &&
                  assertion.getEndDay() == null &&
                  assertion.getEndMonth() == null &&
                  assertion.getEndYear() == null &&
                  assertion.getExternalId() == null &&
                  assertion.getExternalIdType() == null &&
                  assertion.getExternalIdUrl() == null &&
                  assertion.getLastModifiedBy() == null &&
                  assertion.getModified() == null &&
                  assertion.getOrcidError() == null &&
                  assertion.getOrcidId() == null &&
                  assertion.getOrgCity() == null &&
                  assertion.getOrgCity() == null &&
                  assertion.getOrgCountry() == null &&
                  assertion.getOrgName() == null &&
                  assertion.getOrgRegion() == null &&
                  assertion.getOwnerId() == null &&
                  assertion.getPutCode() == null &&
                  assertion.getRoleTitle() == null &&
                  assertion.getSalesforceId() == null &&
                  assertion.getStartDay() == null &&
                  assertion.getStartMonth() == null &&
      ...
      private void copyFieldsToUpdate(Assertion source, Assertion destination) {
            ...
            destination.setDisambiguationSource(
                  source.getDisambiguationSource()
            );
      ...
      public void postAssertionsToOrcid() throws JAXBException {
            ...
            List<Assertion> assertionsToAdd =
                  assertionRepository.findAllToCreateInOrcidRegistry(pageable);
            while (assertionsToAdd != null && !assertionsToAdd.isEmpty()) {
                  for (Assertion assertion : assertionsToAdd) {
                        try {
                              ...
                              LOG.error(
                                    "Unexpected error POSTing assertion to registry",
                                    e
                              );
                        }
                  }
      ...
      public void postAssertionToOrcid(Assertion assertion)
            throws JAXBException {
            Optional<OrcidRecord> record = orcidRecordService.findOneByEmail(
                  assertion.getEmail()
            );
            AssertionStatus deniedStatus = checkForTokenDeniedStatus(
                  record,
                  assertion
            );

            if (
                  tokenAndOrcidIdAvailable(record, assertion) &&
                  deniedStatus == null
            ) {
                  OrcidRecord orcidRecord = record.get();
                  String idToken = orcidRecord.getToken(
                        assertion.getSalesforceId(),
                        false
                  );
                  ...
                  try {
                        String putCode = postToOrcidRegistry(
                              orcid,
                              assertion,
                              idToken
                        );
                        ...
                        LOG.error(
                              "Error posting assertion " + assertion.getId(),
                              e
                        );
                        storeError(
                              assertion,
      ...
      public void putAssertionsInOrcid() throws JAXBException {
            ...
            Pageable pageable = getPageableForRegistrySync();
            List<Assertion> assertionsToUpdate =
                  assertionRepository.findAllToUpdateInOrcidRegistry(pageable);
            while (
                  assertionsToUpdate != null && !assertionsToUpdate.isEmpty()
            ) {
                  for (Assertion assertion : assertionsToUpdate) {
                        Assertion refreshed = assertionRepository
                              .findById(assertion.getId())
                              .get();
                        try {
                              putAssertionInOrcid(refreshed);
                        } catch (Exception e) {
                              LOG.error(
                                    "Unexpected error PUTting assertion in registry",
                                    e
                              );
                        }
                  }
      ...
      public void putAssertionInOrcid(Assertion assertion)
            throws JAXBException {
            Optional<OrcidRecord> record = orcidRecordService.findOneByEmail(
                  assertion.getEmail()
            );
            ...
            if (
                  tokenAndOrcidIdAvailable(record, assertion) &&
                  !StringUtils.isBlank(assertion.getPutCode()) &&
                  deniedStatus == null
            ) {
                  OrcidRecord orcidRecord = record.get();
                  String orcid = orcidRecord.getOrcid();
                  ...
                  try {
                        ...
                        LOG.error(
                              "Error with assertion " + assertion.getId(),
                              e
                        );
                        storeError(
                              assertion,
      ...
      private void deleteAssertionFromOrcidRegistry(Assertion assertion)
            throws RegistryDeleteFailureException {
            Optional<OrcidRecord> record = orcidRecordService.findOneByEmail(
                  assertion.getEmail()
            );
            ...
            try {
                  LOG.info(
                        "Exchanging id token for {}",
                        record.get().getOrcid()
                  );
                  String accessToken = orcidAPIClient.exchangeToken(
                        record.get().getToken(assertion.getSalesforceId(), true)
                  );
                  orcidAPIClient.deleteAffiliation(
                        record.get().getOrcid(),
                        accessToken,
                        assertion
                  );
      ...
      private void checkAssertionAccess(
            Assertion assertion,
            String salesforceId
      ) {
            if (!salesforceId.equals(assertion.getSalesforceId())) {
                  throw new BadRequestAlertException(
                        "This affiliations doesnt belong to your organization",
                        "affiliation",
                        "affiliationOtherOrganization"
                  );
            }
      ...
      public void generateAssertionsReport() throws IOException {
            ...
            csvReportService.storeCsvReportRequest(
                  assertionsUserService.getLoggedInUserId(),
                  filename,
                  CsvReport.ASSERTIONS_REPORT_TYPE
            );
      }

      public void generateAndSendMemberAssertionStats() throws IOException {
            List<MemberAssertionStatusCount> counts =
                  assertionRepository.getMemberAssertionStatusCounts();
            Map<String, MemberAssertionStats> stats = getMemberAssertionStats(
                  counts
            );
            String reportCsv = getMemberAssertionStatsCsv(stats);
            File writtenFile = storedFileService.storeMemberAssertionStatsFile(
                  reportCsv
            );
            mailService.sendMemberAssertionStatsMail(writtenFile);
      }

      private String getMemberAssertionStatsCsv(
            Map<String, MemberAssertionStats> stats
      ) throws IOException {
      ...
      private Map<String, MemberAssertionStats> getMemberAssertionStats(
            List<MemberAssertionStatusCount> counts
      ) {
            ...
            for (MemberAssertionStatusCount count : counts) {
                  if (!stats.containsKey(count.getSalesforceId())) {
                        MemberAssertionStats memberStats =
                              new MemberAssertionStats();
                        memberStats.setMemberName(
                              memberService.getMemberName(
                                    count.getSalesforceId()
                              )
                        );
                        stats.put(count.getSalesforceId(), memberStats);
                  }
                  stats
                        .get(count.getSalesforceId())
                        .setStatusCount(
                              count.getStatus(),
      ...
      private boolean tokenAndOrcidIdAvailable(
            Optional<OrcidRecord> record,
            Assertion assertion
      ) {
            if (!record.isPresent()) {
                  return false;
            ...
            String idToken = record
                  .get()
                  .getToken(assertion.getSalesforceId(), false);
      ...
      private AssertionStatus checkForTokenDeniedStatus(
            Optional<OrcidRecord> orcidRecord,
            Assertion assertion
      ) {
            if (orcidRecord.isPresent()) {
                  return checkForTokenDeniedStatus(
                        orcidRecord.get(),
                        assertion
                  );
            }
      ...
      private AssertionStatus checkForTokenDeniedStatus(
            ...
            if (
                  orcidRecord.getRevokedDate(assertion.getSalesforceId()) !=
                  null
            ) {
                  return AssertionStatus.USER_REVOKED_ACCESS;
            }
            if (
                  orcidRecord.getDeniedDate(assertion.getSalesforceId()) != null
            ) {
      ...
      private String postToOrcidRegistry(
            String orcid,
            Assertion assertion,
            String idToken
      )
            throws JSONException, ClientProtocolException, IOException, JAXBException {
            LOG.info(
                  "Exchanging id token for access token for assertion {}, orcid {}",
                  assertion.getId(),
                  orcid
            );
            String accessToken = orcidAPIClient.exchangeToken(idToken);
            LOG.info(
                  "POST affiliation for {} and assertion id {}",
                  orcid,
                  assertion.getId()
            );
            return orcidAPIClient.postAffiliation(
                  orcid,
                  accessToken,
                  assertion
            );
      ...
      private void putInOrcidRegistry(
            String orcid,
            Assertion assertion,
            String idToken
      )
            throws JSONException, JAXBException, ClientProtocolException, IOException {
            LOG.info(
                  "Exchanging id token for access token for assertion {}, orcid {}",
                  assertion.getId(),
                  orcid
            ...
            String accessToken = orcidAPIClient.exchangeToken(idToken);
            LOG.info(
                  "PUT affiliation with put-code {} for {} and assertion id {}",
                  assertion.getPutCode(),
                  orcid,
                  assertion.getId()
            );
      ...
      private boolean checkRegistryDeletePreconditions(
            Optional<OrcidRecord> record,
            Assertion assertion
      ) {
            String error = null;
            if (!record.isPresent()) {
                  LOG.error(
                        "OrcidRecord not available for email {}",
                        assertion.getEmail()
                  );
      ...
      private void storeError(
            ...
            int statusCode,
            String error,
            AssertionStatus defaultErrorStatus
      ) {
            LOG.info(
                  "Error updating ORCID registry: status code - {}, error - {}",
                  statusCode,
                  error
            );
            ...
            assertion.setStatus(
                  getErrorStatus(assertion, defaultErrorStatus).name()
            );
            ...
            if (
                  ...
                  LOG.info(
                        "Assertion status set to USER_REVOKED_ACCESS, updating id token accordingly"
                  );
                  orcidRecordService.revokeIdToken(
                        assertion.getEmail(),
                        assertion.getSalesforceId()
                  );
            }
      ...
      private AssertionStatus getErrorStatus(
            Assertion assertion,
            AssertionStatus defaultError
      ) {
      ...
      public List<Assertion> findByEmailAndSalesForceId(
            String email,
            String salesForceId
      ) {
            return assertionRepository.findByEmailAndSalesforceId(
                  email,
                  salesForceId
            );
      }
      ...
      public Optional<Assertion> findOneByEmailIgnoreCase(String email) {
            return assertionRepository.findOneByEmailIgnoreCase(
                  email.toLowerCase()
            );
      }
      ...
      public void updateAssertionStatus(
      ...
      public void generatePermissionLinks() {
            ...
            csvReportService.storeCsvReportRequest(
                  assertionsUserService.getLoggedInUserId(),
                  filename,
                  CsvReport.PERMISSION_LINKS_TYPE
            );
      }
      ...
      public void generateAssertionsCSV() {
            ...
            csvReportService.storeCsvReportRequest(
                  assertionsUserService.getLoggedInUserId(),
                  filename,
                  CsvReport.ASSERTIONS_FOR_EDIT_TYPE
            );
      }
      ...
      public void processAssertionUploads() {
            List<StoredFile> pendingUploads =
      ...
      public void markPendingAssertionsAsNotificationRequested(
            String salesforceId
      ) {
            assertionRepository.updateStatusPendingToNotificationRequested(
                  salesforceId
            );
      }

      public void updateOrcidIdsForEmailAndSalesforceId(
            String email,
            String salesforceId
      ) {
            Optional<OrcidRecord> record = orcidRecordService.findOneByEmail(
            ...
            List<Assertion> assertions = assertionRepository.findAllByEmail(
                  email
            );
            assertions
                  .stream()
                  .filter(a ->
                        a.getOrcidId() == null &&
                        salesforceId.equals(a.getSalesforceId())
                  )
                  .forEach(a -> {
                        a.setOrcidId(orcid);
                        assertionRepository.save(a);
      ...
      private void processAssertionsUploadFile(StoredFile uploadFile) {
            ...
            AssertionServiceUser user = assertionsUserService.getUserById(
                  uploadFile.getOwnerId()
            );
            ...
            try {
                  ...
                  summary.setDate(
                        DATE_FORMAT.format(uploadFile.getDateWritten())
                  );
      ...
      private AssertionsUploadSummary processUpload(
                        ...
                        if (a.getId() == null || a.getId().isEmpty()) {
                              ...
                              if (
                                    !user
                                          .getSalesforceId()
                                          .equals(
                                                existingAssertion.getSalesforceId()
                                          )
                              ) {
                                    throw new BadRequestAlertException(
                                          "This affiliation doesn't belong to your organization",
                                          "affiliation",
                                          "affiliationOtherOrganization"
                                    );
                              }
      ...
      private AssertionsUpload readUpload(
            File file,
            AssertionServiceUser user
      ) {
      ...
      private Page<Assertion> findBySalesforceId(
            String salesforceId,
            Pageable pageable
      ) {
            Page<Assertion> assertions = assertionRepository.findBySalesforceId(
                  salesforceId,
                  pageable
            );
            setPrettyStatus(assertions);
      ...
      private void setPrettyStatus(Assertion assertion) {
            if (assertion.getStatus() != null) {
                  assertion.setPrettyStatus(
                        AssertionStatus
                              .valueOf(assertion.getStatus())
                              .getValue()
                  );
            }
      }

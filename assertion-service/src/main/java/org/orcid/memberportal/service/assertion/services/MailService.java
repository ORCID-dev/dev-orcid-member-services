...
public class MailService {
    ...
    public void sendCsvReportMail(
        ...
        try {
            ...
            LOGGER.error(
                "Error sending csv report email to {}",
                user.getEmail(),
                e
            );
        }
    ...
    public void sendMemberAssertionStatsMail(File stats) {
        LOGGER.debug(
            "Sending member stats email to '{}'",
            applicationProperties.getMemberAssertionStatsRecipient()
        );
        Context context = new Context(Locale.ENGLISH);
        String content = templateEngine.process(
            "mail/memberAssertionStats",
            context
        );
        String subject = messageSource.getMessage(
            "email.memberAssertionStats.title",
            null,
            Locale.ENGLISH
        );
        try {
    ...
    public void sendAssertionsUploadSummaryMail(
        AssertionsUploadSummary summary,
        AssertionServiceUser user
    ) {
        Locale locale = LocaleUtils.getLocale(user.getLangKey());
        Context context = new Context(locale);
        ...
        try {
            ...
            LOGGER.error(
                "Error sending csv upload summary email to {}",
                user.getEmail(),
                e
            );
        }
    ...
    public void sendNotificationsSummary(
        AssertionServiceUser user,
        ...
        String content = templateEngine.process(
            "mail/notificationsSummary",
            context
        );
        String subject = messageSource.getMessage(
            "email.notificationsSummary.title",
            null,
            locale
        );
        try {
            ...
            LOGGER.error(
                "Error sending notifications summary email to {}",
                user.getEmail(),
                e
            );
        }

...
public class NotificationService {
      ...
      public void createSendNotificationsRequest(
            ...
            if (findActiveRequestBySalesforceId(salesforceId) != null) {
                  throw new RuntimeException(
                        "Send notifications request already active for " +
                        salesforceId
                  );
            }
      ...
      private void processRequest(SendNotificationsRequest request) {
            ...
            String orgName = memberService.getMemberName(
                  request.getSalesforceId()
            );
            ...
            while (assertions != null && !assertions.isEmpty()) {
                  assertions.forEach(a -> {
                        if (
                              ...
                              Instant firstSent = a.getNotificationSent() !=
                                    null
                                    ? a.getNotificationSent()
                                    : a.getInvitationSent();
                              Instant lastSent = a.getNotificationLastSent() !=
                                    null
      ...
      private void findAssertionsAndAttemptSend(
            ...
            try {
                  ...
                  if (orcidId == null) {
                        ...
                        LOG.info(
                              "ORCID id found for {}. Sending notification.",
                              email
                        );
                        sendNotification(
                              email,
      ...
      private void sendNotification(
            ...
            allAssertionsForEmailAndMember.forEach(a -> {
                  ...
                  Instant now = Instant.now();
                  if (
                        a.getNotificationSent() == null &&
                        a.getInvitationSent() == null
                  ) {
      ...
      private NotificationPermission getPermissionLinkNotification(
            ...
            notificationPermission.setNotificationType(

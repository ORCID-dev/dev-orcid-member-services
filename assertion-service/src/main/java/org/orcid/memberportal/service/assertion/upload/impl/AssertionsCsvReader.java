...
public class AssertionsCsvReader implements AssertionsUploadReader {
      ...
      private Assertion processExternalId(CSVRecord line, Assertion a) {
            ...
            a.setExternalIdType(
                  getOptionalNullableValue(line, "external-id-type")
            );
            a.setExternalIdUrl(
                  getOptionalNullableValue(line, "external-id-url")
            );
            return a;
      }
      ...
      private Assertion processDisambiguationSource(
            ...
            if (
                  getOptionalNullableValue(line, "disambiguation-source") ==
                  null
            ) {
                  upload.addError(
                        line.getRecordNumber(),
      ...
      private AssertionsUpload.AssertionsUploadDate getDate(
            ...
            if (date != null && !StringUtils.isBlank(date)) {
                  ...
                  String dayToValidate = dateParts.length > 2
                        ? dateParts[2]
                        : "0";
                  if (
                        validDate(
      ...
      private Assertion processDepartmentName(CSVRecord line, Assertion a) {
            a.setDepartmentName(
                  getOptionalNullableValue(line, "department-name")
            );
            return a;
      }
      ...
      private Assertion processAffiliationSection(
            ...
            if (
                  affiliationSectionValue == null ||
                  affiliationSectionValue.isEmpty()
            ) {
                  upload.addError(
                        line.getRecordNumber(),
                  ...
                  if (
                        "INVITED-POSITION".equals(
                                    affiliationSectionValue.toUpperCase()
                              )
                  ) {
                        affiliationSection =
                              AffiliationSection.INVITED_POSITION;
                  } else {
                        affiliationSection =
      ...
      private void checkStartDateBeforeEndDate(
            ...
            AssertionsUploadDate endDate = getDate(
                  line,
                  upload,
                  "end-date",
                  user

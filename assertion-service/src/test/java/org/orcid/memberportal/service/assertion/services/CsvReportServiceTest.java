...
class CsvReportServiceTest {
  ...
  void testProcessCsvReports() throws IOException {
    ...
    Mockito
      .when(storedFileService.storeCsvReportFile(Mockito.eq("report"), Mockito.eq("file.csv"), Mockito.any(AssertionServiceUser.class)))
      .thenReturn(getDummyStoredfile());
    Mockito
      .when(
        messageSource.getMessage(Mockito.eq("email.csvReport.affiliationsForEdit.subject"), Mockito.isNull(), Mockito.any(Locale.class))
      )
      .thenReturn("edit subject");
    Mockito
      .when(
        messageSource.getMessage(Mockito.eq("email.csvReport.affiliationsForEdit.content"), Mockito.isNull(), Mockito.any(Locale.class))
      )
      .thenReturn("edit content");
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.csvReport.permissionLinks.subject"), Mockito.isNull(), Mockito.any(Locale.class)))
      .thenReturn("links subject");
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.csvReport.permissionLinks.content"), Mockito.isNull(), Mockito.any(Locale.class)))
      .thenReturn("links content");
    Mockito
      .when(
        messageSource.getMessage(Mockito.eq("email.csvReport.affiliationStatusReport.subject"), Mockito.isNull(), Mockito.any(Locale.class))
      )
      .thenReturn("report subject");
    Mockito
      .when(
        messageSource.getMessage(Mockito.eq("email.csvReport.affiliationStatusReport.content"), Mockito.isNull(), Mockito.any(Locale.class))
      )
      .thenReturn("report content");
    Mockito
      .doNothing()
      .when(mailService)
      .sendCsvReportMail(Mockito.any(File.class), Mockito.any(AssertionServiceUser.class), Mockito.anyString(), Mockito.anyString());

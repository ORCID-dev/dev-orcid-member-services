...
class CsvReportServiceTest {
    ...
    void testProcessCsvReports() throws IOException {
        Mockito
            .when(csvReportRepository.findAllUnprocessed())
            .thenReturn(getUnprocessedCsvReports());
        Mockito
            .when(userService.getUserById(Mockito.eq("user")))
            .thenReturn(getDummyUser());
        Mockito
            .when(assertionsReportCsvWriter.writeCsv(Mockito.eq("salesforce")))
            .thenReturn("report");
        Mockito
            .when(assertionsForEditCsvWriter.writeCsv(Mockito.eq("salesforce")))
            .thenReturn("report");
        Mockito
            .when(permissionLinksCsvWriter.writeCsv(Mockito.eq("salesforce")))
            .thenReturn("report");
        Mockito
            .when(
                storedFileService.storeCsvReportFile(
                    Mockito.eq("report"),
                    Mockito.eq("file.csv"),
        ...
        Mockito
            .when(
                messageSource.getMessage(
                    Mockito.eq("email.csvReport.affiliationsForEdit.subject"),
                    Mockito.isNull(),
                    Mockito.any(Locale.class)
                )
            )
            .thenReturn("edit subject");
        Mockito
            .when(
                messageSource.getMessage(
                    Mockito.eq("email.csvReport.affiliationsForEdit.content"),
                    Mockito.isNull(),
                    Mockito.any(Locale.class)
                )
            )
            .thenReturn("edit content");
        Mockito
            .when(
                messageSource.getMessage(
                    Mockito.eq("email.csvReport.permissionLinks.subject"),
                    Mockito.isNull(),
                    Mockito.any(Locale.class)
                )
            )
            .thenReturn("links subject");
        Mockito
            .when(
                messageSource.getMessage(
                    Mockito.eq("email.csvReport.permissionLinks.content"),
                    Mockito.isNull(),
                    Mockito.any(Locale.class)
                )
            )
            .thenReturn("links content");
        Mockito
            .when(
                messageSource.getMessage(
                    Mockito.eq(
        ...
        Mockito
            .doNothing()
            .when(storedFileService)
            .markAsProcessed(Mockito.any(StoredFile.class));

        csvReportService.processCsvReports();

        Mockito
            .verify(assertionsReportCsvWriter)
            .writeCsv(Mockito.eq("salesforce"));
        Mockito
            .verify(assertionsForEditCsvWriter)
            .writeCsv(Mockito.eq("salesforce"));
        Mockito
            .verify(permissionLinksCsvWriter)
            .writeCsv(Mockito.eq("salesforce"));
        Mockito
            .verify(mailService)
            .sendCsvReportMail(
                Mockito.any(File.class),
                Mockito.any(AssertionServiceUser.class),
                Mockito.eq("edit subject"),
                Mockito.eq("edit content")
            );
        Mockito
            .verify(mailService)
            .sendCsvReportMail(
                Mockito.any(File.class),
                Mockito.any(AssertionServiceUser.class),
                Mockito.eq("links subject"),
                Mockito.eq("links content")
            );
        Mockito
            .verify(mailService)
            .sendCsvReportMail(
                Mockito.any(File.class),
                Mockito.any(AssertionServiceUser.class),
                Mockito.eq("report subject"),
                Mockito.eq("report content")
            );
        Mockito
            .verify(storedFileService, Mockito.times(3))
            .markAsProcessed(Mockito.any(StoredFile.class));
    }
    ...
    void testProcessCsvReportsWithError() throws IOException {
        Mockito
            .when(csvReportRepository.findAllUnprocessed())
            .thenReturn(
                Arrays.asList(getCsvReport(CsvReport.PERMISSION_LINKS_TYPE))
            );
        Mockito
            .when(userService.getUserById(Mockito.eq("user")))
            .thenReturn(getDummyUser());
        ...
        Mockito
            .verify(storedFileService, Mockito.never())
            .markAsProcessed(Mockito.any(StoredFile.class));
    ...
    void testStoreCsvReportRequest() {
        Mockito
            .when(csvReportRepository.save(Mockito.any(CsvReport.class)))
            .thenReturn(new CsvReport());
        ...
        assertThat(stored.getReportType())
            .isEqualTo(CsvReport.ASSERTIONS_REPORT_TYPE);
    }

    private List<CsvReport> getUnprocessedCsvReports() {
        return Arrays.asList(
            getCsvReport(CsvReport.PERMISSION_LINKS_TYPE),
            getCsvReport(CsvReport.ASSERTIONS_REPORT_TYPE),

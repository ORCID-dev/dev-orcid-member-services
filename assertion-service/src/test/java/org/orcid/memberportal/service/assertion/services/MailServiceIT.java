...
class MailServiceIT {
  ...
  public void setUp() {
    ...
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.memberAssertionStats.title"), Mockito.isNull(), Mockito.any(Locale.class)))
      .thenReturn("member stats");
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.affiliationUploadSummary.title"), Mockito.isNull(), Mockito.any(Locale.class)))
      .thenReturn("summary");
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.notificationsSummary.title"), Mockito.isNull(), Mockito.any(Locale.class)))
      .thenReturn("notifications summary");
    Mockito
      .when(messageSource.getMessage(Mockito.eq("email.invitation.title"), Mockito.any(), Mockito.any(Locale.class)))
      .thenReturn("someone wants to add something to your record");
  }
  ...
  void testSendCsvReportMail() throws MailException {
    Mockito
      ...
      .sendMailWithAttachment(
        Mockito.eq("memberstats@orcid.org"),
        Mockito.eq("member stats"),
        Mockito.eq("something"),
    ...
    Mockito
      ...
      .sendMailWithAttachment(recipientCaptor.capture(), subjectCaptor.capture(), Mockito.anyString(), fileCaptor.capture());

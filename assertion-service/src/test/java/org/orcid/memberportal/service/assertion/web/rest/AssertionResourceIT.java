...
public class AssertionResourceIT {
  ...
  public void testGetAssertions() throws Exception {
    ...
    MvcResult result = restUserMockMvc
      .perform(
        get("/api/assertions").param("size", "50").accept(TestUtil.APPLICATION_JSON_UTF8).contentType(TestUtil.APPLICATION_JSON_UTF8)
      )
  ...
  public void testSendNotifications() throws Exception {
    restUserMockMvc
      .perform(post("/api/assertion/notification-request").content("{ \"language\":\"en\" }").contentType(MediaType.APPLICATION_JSON))
      .andExpect(status().isOk());

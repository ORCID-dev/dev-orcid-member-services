...
public class SwaggerBasePathRewritingFilter extends SendResponseFilter {
    ...
    private final Logger log = LoggerFactory.getLogger(
        SwaggerBasePathRewritingFilter.class
    );
        ...
        if (context.getResponseGZipped()) {
            try {
                context.setResponseDataStream(
                    new ByteArrayInputStream(gzipData(rewrittenResponse))
                );
            } catch (IOException e) {
    ...
    private String rewriteBasePath(RequestContext context) {
        ...
        try {
            if (context.getResponseGZipped()) {
                responseDataStream =
                    new GZIPInputStream(context.getResponseDataStream());
            }
            ...
            if (response != null) {
                LinkedHashMap<String, Object> map =
                    this.mapper.readValue(response, LinkedHashMap.class);

                String basePath = requestUri.replace(
                    Swagger2Controller.DEFAULT_URL,
                    ""
                );
                map.put("basePath", basePath);
                log.debug(
                    "Swagger-docs: rewritten Base URL with correct micro-service route: {}",
                    basePath
                );

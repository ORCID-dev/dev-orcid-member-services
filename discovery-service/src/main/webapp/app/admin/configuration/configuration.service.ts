...
export class ConfigurationService {
  ...
  getBeans(prefix: String = ''): Observable<Bean[]> {
    return this.http
      .get<ConfigProps>(
        (SERVER_API_URL as string) + prefix + 'management/configprops'
      )
      .pipe(
        map(configProps =>
          Object.values(
  ...
  getPropertySources(prefix: String = ''): Observable<PropertySource[]> {
    return this.http
      .get<Env>((SERVER_API_URL as string) + prefix + 'management/env')
      .pipe(
        map(env => {
  ...
  getInstancePropertySources(
    instance: Route | undefined
  ): Observable<PropertySource[]> {
    if (instance && instance.prefix && instance.prefix.length > 0) {

...
export class LogsService {
  ...
  changeLevel(name: string, configuredLevel: Level): Observable<{}> {
    return this.http.post(SERVER_API_URL + 'management/loggers/' + name, {
      configuredLevel,
    })
  }
  ...
  changeInstanceLevel(
    ...
    for (let i = 0; i < instances.length; i++) {
      if (
        instances[i] &&
        instances[i].prefix &&
        instances[i].prefix.length > 0
      ) {
        changeInstanceLevelResponses.push(
          this.http.post(instances[i].prefix + '/management/loggers/' + name, {
            configuredLevel,
          })
        )
      } else {
        changeInstanceLevelResponses.push(
          this.changeLevel(name, configuredLevel)
        )
      }
    }
  ...
  findInstanceAll(instance: Route | undefined): Observable<LoggersResponse> {
    if (instance && instance.prefix && instance.prefix.length > 0) {
      return this.http.get<LoggersResponse>(
        instance.prefix + '/management/loggers'
      )
    }
    return this.findAll()
  }

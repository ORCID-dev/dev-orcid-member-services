...
import {
  ISFRawMemberOrgIds,
  SFMemberOrgIds,
} from 'app/shared/model/salesforce-member-org-id.model'
...
export class MSMemberService {
  ...
  create(msMember: IMSMember): Observable<EntityResponseType> {
    ...
    return this.http
      .post<IMSMember>(`${this.resourceUrl}/members`, copy, {
        observe: 'response',
      })
  ...
  update(msMember: IMSMember): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(msMember)
    return this.http
      .put<IMSMember>(`${this.resourceUrl}/members`, copy, {
        observe: 'response',
      })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)))
  }
  ...
  validate(msMember: IMSMember): Observable<any> {
    const copy = this.convertDateFromClient(msMember)
    return this.http
      .post<IMSMember>(`${this.resourceUrl}/members/validate`, copy, {
        observe: 'response',
      })
  ...
  find(id: string): Observable<EntityResponseType> {
    return this.http
      .get<IMSMember>(`${this.resourceUrl}/members/${id}`, {
        observe: 'response',
      })
      .pipe(
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http
      .get<IMSMember[]>(`${this.resourceUrl}/members`, {
        params: options,
        observe: 'response',
      })
      .pipe(
  ...
  getAllMembers(): Observable<EntityArrayResponseType> {
    return this.http
      .get<IMSMember[]>(`${this.resourceUrl}/members/list/all`, {
        observe: 'response',
      })
      .pipe(
        map((res: EntityArrayResponseType) =>
  ...
  getMember(): Observable<SFMemberData> {
    return this.http
      .get<ISFRawMemberData>(`${this.resourceUrl}/member-details`, {
        observe: 'response',
      })
      .pipe(
        catchError(err => {
        ...
        map((res: SalesforceDataResponseType) =>
          this.convertToSalesforceMemberData(res)
        ),
        switchMap(value => {
          if (value && !value.id) {
  ...
  addConsortiumMember(
    consortiumMember: ISFNewConsortiumMember
  ): Observable<Boolean> {
    return this.http
      .post<ISFMemberContactUpdate>(
        `${this.resourceUrl}/members/add-consortium-member`,
        consortiumMember,
        { observe: 'response' }
      )
      .pipe(
  ...
  removeConsortiumMember(
    consortiumMember: ISFConsortiumMemberData
  ): Observable<Boolean> {
    return this.http
      .post<ISFMemberContactUpdate>(
        `${this.resourceUrl}/members/remove-consortium-member`,
        consortiumMember,
        { observe: 'response' }
      )
      .pipe(
  ...
  getMemberContacts(): Observable<SFMemberContact[]> {
    return this.http
      .get<ISFRawMemberContacts>(`${this.resourceUrl}/member-contacts`, {
        observe: 'response',
      })
      .pipe(
        takeUntil(this.stopFetchingMemberData),
        map((res: SalesforceContactsResponseType) =>
          this.convertToSalesforceMemberContacts(res)
        ),
        tap(res =>
  ...
  updateContact(contact: ISFMemberContactUpdate): Observable<Boolean> {
    return this.http
      .post<ISFMemberContactUpdate>(
        `${this.resourceUrl}/members/contact-update`,
        contact,
        { observe: 'response' }
      )
      .pipe(
  ...
  getMemberOrgIds(): Observable<SFMemberOrgIds> {
    return this.http
      .get<ISFRawMemberOrgIds>(`${this.resourceUrl}/member-org-ids`, {
        observe: 'response',
      })
      .pipe(
        takeUntil(this.stopFetchingMemberData),
        map((res: SalesforceOrgIdResponseType) =>
          this.convertToMemberOrgIds(res)
        ),
        tap(res =>
  ...
  delete(id: string): Observable<HttpResponse<any>> {
    return this.http.delete<any>(`${this.resourceUrl}/members/${id}`, {
      observe: 'response',
    })
  }

  updatePublicDetails(
    publicDetails: ISFPublicDetails
  ): Observable<HttpResponse<any>> {
    return this.http.put(`${this.resourceUrl}/public-details`, publicDetails, {
      observe: 'response',
    })
  }
        ...
        tap(r => {
          if (r && r.body) {
            this.memberData.next({
              ...
              consortiumLeadName: r.body.clientName,
            })
          }
        })
      )
  ...
  getIsConsortiumLead(
    salesforceId: string
  ): Observable<EntityResponseType | never> {
    if (salesforceId) {
  ...
  protected convertDateFromClient(msMember: IMSMember): IMSMember {
    const copy: IMSMember = Object.assign({}, msMember, {
      createdDate:
        msMember.createdDate != null && msMember.createdDate.isValid()
          ? msMember.createdDate.toJSON()
          : null,
      lastModifiedDate:
        msMember.lastModifiedDate != null && msMember.lastModifiedDate.isValid()
          ? msMember.lastModifiedDate.toJSON()
          : null,
    })
    return copy
  }
  ...
  protected convertDateArrayFromServer(
    ...
    if (res.body) {
      res.body.forEach((msMember: IMSMember) => {
        msMember.createdDate =
          msMember.createdDate != null ? moment(msMember.createdDate) : null
        msMember.lastModifiedDate =
          msMember.lastModifiedDate != null
            ? moment(msMember.lastModifiedDate)
            : null
      })
    }
    return res
  ...
  protected convertToSalesforceMemberData(
    res: SalesforceDataResponseType
  ): SFMemberData {
    if (res.body) {
      return {
        ...new SFMemberData(),
  ...
  protected convertToSalesforceMemberContacts(
    res: SalesforceContactsResponseType
  ): SFMemberContact[] {
    const contacts = {}
    if (res.body && res.body.records.length > 0) {
      for (const contact of res.body.records) {
        ...
        if (!contacts[contact.Contact_Curr_Email__c]) {
          contacts[contact.Contact_Curr_Email__c] =
            this.convertToSalesforceMemberContact(contact)
        }
        if (contact.Voting_Contact__c) {
          contacts[contact.Contact_Curr_Email__c].memberOrgRole.push(
            'Voting contact'
          )
        }
        contacts[contact.Contact_Curr_Email__c].memberOrgRole.unshift(
          contact.Member_Org_Role__c
        )
      }
  ...
  protected convertToSalesforceMemberContact(
    res: ISFRawMemberContact
  ): SFMemberContact {
    return {
      ...new SFMemberContact(),
  ...
  protected convertToMemberOrgIds(
    res: SalesforceOrgIdResponseType
  ): SFMemberOrgIds {
    if (res.body && res.body.records.length > 0) {
  ...
  protected convertToConsortiumMembers(
    ...
    for (const consortiumOpportunity of consortiumOpportunities) {
      consortiumMembers.push(
        this.convertToConsortiumMember(consortiumOpportunity)
      )
    }
    return consortiumMembers
  }

  protected convertToConsortiumMember(
    consortiumOpportunity: ISFRawConsortiumMemberData
  ): SFConsortiumMemberData {
    const consortiumMember: SFConsortiumMemberData =
      new SFConsortiumMemberData()
    consortiumMember.orgName =
      consortiumOpportunity.Account.Public_Display_Name__c

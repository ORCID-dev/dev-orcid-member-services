...
export class MSUserUpdateComponent implements OnInit {
  ...
  onChanges(): void {
    this.editForm.get('salesforceId').valueChanges.subscribe(val => {
      const selectedOrg = this.memberList.find(
        cm => cm.salesforceId === this.editForm.get(['salesforceId']).value
      )
  ...
  updateForm(msUser: IMSUser) {
    this.editForm.patchValue({
      ...
      createdDate:
        msUser.createdDate != null
          ? msUser.createdDate.format(DATE_TIME_FORMAT)
          : null,
      lastModifiedBy: msUser.lastModifiedBy,
      lastModifiedDate:
        msUser.lastModifiedDate != null
          ? msUser.lastModifiedDate.format(DATE_TIME_FORMAT)
          : null,
    })
  ...
  validateOrgOwners() {
    this.isSaving = true
    this.msUserService
      .hasOwner(this.editForm.get('salesforceId').value)
      .subscribe(value => {
        this.isSaving = false
        ...
        if (data.valid) {
          if (msUser.id !== undefined) {
            if (this.currentAccount.id === msUser.id) {
              if (this.currentAccount.mainContact !== msUser.mainContact) {
                this.subscribeToUpdateResponseWithOwnershipChange(
                  this.msUserService.update(msUser)
                )
              } else {
            ...
            if (msUser.mainContact && !this.hasRoleAdmin()) {
              this.subscribeToSaveResponseWithOwnershipChange(
                this.msUserService.create(msUser)
              )
            } else {
    ...
    this.msUserService.sendActivate(this.existentMSUser).subscribe(res => {
      if (res.ok) {
        this.alertService.success(
          'gatewayApp.msUserServiceMSUser.sendActivate.success.string',
          null,
          null
        )
      } else {
        this.alertService.success(
          'gatewayApp.msUserServiceMSUser.sendActivate.error.string',
          null,
          null
        )
      }
  ...
  displaySendActivate() {
    if (
      this.existentMSUser &&
      this.existentMSUser.email &&
      !this.existentMSUser.activated
    ) {
  ...
  private createFromForm(): IMSUser {
    return {
      ...
      isAdmin: this.editForm.get(['isAdmin'])
        ? this.editForm.get(['isAdmin']).value
        : false,
  ...
  protected subscribeToUpdateResponse(
    result: Observable<HttpResponse<IMSUser>>
  ) {
    result.subscribe(
  ...
  protected subscribeToSaveResponseWithOwnershipChange(
    result: Observable<HttpResponse<IMSUser>>
  ) {
    result.subscribe(
  ...
  protected subscribeToUpdateResponseWithOwnershipChange(
    result: Observable<HttpResponse<IMSUser>>
  ) {
    result.subscribe(

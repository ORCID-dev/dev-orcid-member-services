...
export class MSUserService {
  ...
  validate(msUser: IMSUser): Observable<HttpResponse<any>> {
    const copy = this.convertDateFromClient(msUser)
    return this.http
      .post<IMSUser>(this.resourceUrl + '/validate', copy, {
        observe: 'response',
      })
      .pipe(map((res: EntityResponseType) => this.convertFromServer(res)))
  }
  ...
  sendActivate(msUser: IMSUser): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(msUser)
    return this.http.post<IMSUser>(
      `${this.resourceUrl}/${msUser.id}/sendActivate`,
      copy,
      { observe: 'response' }
    )
  }
  ...
  findBySalesForceId(
    ...
    return this.http
      .get<IMSUser[]>(`${this.resourceUrl}/salesforce/${salesforceId}/p`, {
        params: options,
        observe: 'response',
      })
      .pipe(
        map((res: EntityArrayResponseType) => this.convertArrayFromServer(res))
      )
  }
  ...
  protected convertDateFromClient(msUser: IMSUser): IMSUser {
    const copy: IMSUser = Object.assign({}, msUser, {
      createdDate:
        msUser.createdDate != null && msUser.createdDate.isValid()
          ? msUser.createdDate.toJSON()
          : null,
      lastModifiedDate:
        msUser.lastModifiedDate != null && msUser.lastModifiedDate.isValid()
          ? msUser.lastModifiedDate.toJSON()
          : null,
    })
    return copy
  }
  ...
  protected convertArrayFromServer(
    res: EntityArrayResponseType
  ): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((msUser: IMSUser) => {
        msUser.createdDate =
          msUser.createdDate != null ? moment(msUser.createdDate) : null
        msUser.lastModifiedDate =
          msUser.lastModifiedDate != null
            ? moment(msUser.lastModifiedDate)
            : null

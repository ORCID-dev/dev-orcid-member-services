...
export class ContactUpdateComponent implements OnInit, OnDestroy {
    ...
    this.memberDataSubscription = this.memberService.memberData.subscribe(
      data => {
        ...
        if (data.contacts && this.contactId) {
          this.contact = Object.values(data.contacts).find(
            contact => contact.contactEmail == this.contactId
          )
  ...
  validateContactRoles(rolesArray: FormArray): ValidationErrors | null {
    const selectedRoles = rolesArray.controls.filter(
      control => control.value.selected
    )
  ...
  onSaveSuccess() {
    ...
    this.alertService.showHomepageLightboxModal({
      alertComponent: ContactUpdateConfirmationAlert,
    })

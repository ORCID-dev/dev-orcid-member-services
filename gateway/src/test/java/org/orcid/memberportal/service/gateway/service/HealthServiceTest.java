...
public class HealthServiceTest {
    ...
    void testCheckGlobalHealth_notAllHealthy() throws IOException {
        Mockito
            .when(
                healthClient.getHealth(
                    Mockito.eq(
                        "http://localhost:8080/services/userservice/management/health"
                    )
                )
            )
            .thenReturn(healthy());
        Mockito
            .when(
                healthClient.getHealth(
                    Mockito.eq(
                        "http://localhost:8080/services/assertionservice/management/health"
                    )
                )
            )
            .thenReturn(healthy());
        Mockito
            .when(
                healthClient.getHealth(
                    Mockito.eq(
                        "http://localhost:8080/services/memberservice/management/health"
                    )
                )
            )
        ...
        Mockito
            .when(
                healthClient.getHealth(
                    Mockito.eq("http://localhost:8080/management/health")
                )
            )
            .thenReturn(healthy());

        CompositeHealthDTO checkedHealth = healthService.checkGlobalHealth(
            getRoutes(),
        ...
        Mockito
            .verify(healthClient, Mockito.times(4))
            .getHealth(Mockito.anyString()); // includes call to get gateway health
    }

    @Test
    void testCheckGlobalHealth_allHealthy() throws IOException {
        Mockito
            .when(healthClient.getHealth(Mockito.anyString()))
            .thenReturn(healthy());
    ...
    void testCheckGlobalHealth_WithError() throws IOException {
        ...
        Mockito
            ...
            .thenThrow(new IOException("overall status should be DOWN"));
    ...
    void testCheckGlobalHealth_OneMissing() throws IOException {
        ...
        Mockito
            .verify(healthClient, Mockito.times(3))
            .getHealth(Mockito.anyString()); // includes call to get gateway health
    }

    private List<Route> getRoutes() {
        Route userService = new Route(
            "userservice",
            null,
            null,
        ...
        Route assertionService = new Route(
            "assertionservice",
            null,
            null,
            "/services/assertionservice",
            false,
            null
        );

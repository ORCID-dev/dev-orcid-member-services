...
class UserServiceTest {
    ...
    void testResendActivationEmail() {
        ...
        user.setResetDate(
            Instant
                .now()
                .minusSeconds(UserService.RESET_KEY_LIFESPAN_IN_SECONDS + 10000)
        );
        Mockito
            .when(userRepository.findOneByResetKey(Mockito.eq("key")))
            .thenReturn(Optional.of(user));
        Mockito
            .doNothing()
            .when(mailService)
            .sendActivationEmail(Mockito.any(User.class));
    ...
    void testClearUser() {
        ...
        Mockito
            .when(userRepository.findOneById(Mockito.eq("some-id")))
            .thenReturn(Optional.of(userToBeCleared));
        Mockito
    ...
    void testCreateUserForMemberWithAsserionsEnabled() {
        ...
        Mockito
            ...
            .toUser(Mockito.any(UserDTO.class));
    }
    ...
    public void testUpdateUserNoOrgOwnerChange() {
        ...
        Mockito
            ...
            .sendOrganizationOwnerChangedMail(
    ...
    public void testUpdateUserWithOrgOwnerChange() {
        ...
        Mockito
            ...
            .thenReturn(new UserDTO());
        Mockito
            .when(memberService.getMemberNameBySalesforce(Mockito.anyString()))
            .thenReturn("member");
    ...
    public void testUploadUserCSV() throws IOException {
        Mockito
            .when(
                usersUploadReader.readUsersUpload(
        ...
        Mockito
            .when(
                userRepository.findOneByEmailIgnoreCase(
                    Mockito.eq("user2@orcid.org")
                )
            )
            .thenReturn(Optional.of(getUser("user2@orcid.org")));
        Mockito
            .when(userRepository.findOneById(Mockito.eq("user2@orcid.org")))
            .thenReturn(Optional.of(getUser("user2@orcid.org")));
        Mockito
            .when(userMapper.toUser(Mockito.any(UserDTO.class)))
            .thenReturn(new User());
    ...
    public void testExpiredResetKey() {
        ...
        user.setResetDate(
            Instant
                .now()
                .minusSeconds(UserService.RESET_KEY_LIFESPAN_IN_SECONDS - 2000)
        );
        Mockito
            .when(userRepository.findOneByResetKey(Mockito.anyString()))
            .thenReturn(Optional.of(user));
    ...
    public void testUpdateMemberNames_fullPage() {
        Mockito
            .when(
                userRepository.findByMemberName(
        ...
        Mockito
            .verify(userRepository, Mockito.times(1))
            .findByMemberName(Mockito.any(Pageable.class), Mockito.isNull());
        Mockito
            .verify(memberService, Mockito.times(10))
            .getMemberNameBySalesforce(Mockito.anyString());
    }
    ...
    public void testUpdateMemberNames_partPage() {
        ...
        Mockito
            ...
            .findByMemberName(Mockito.any(Pageable.class), Mockito.isNull());
        Mockito
            .verify(memberService, Mockito.times(2))
            .getMemberNameBySalesforce(Mockito.anyString());
    }
    ...
    public void testGetAllManagedUsers() {
        Mockito
            .when(
                userRepository.findByDeletedFalse(Mockito.any(Pageable.class))
            )
            .thenReturn(new PageImpl<>(getListOfUsers(20)));
        Mockito
            .when(
                userRepository.findByDeletedIsFalseAndMemberNameContainingIgnoreCaseOrDeletedIsFalseAndFirstNameContainingIgnoreCaseOrDeletedIsFalseAndLastNameContainingIgnoreCaseOrDeletedIsFalseAndEmailContainingIgnoreCase(
                    Mockito.eq("filter"),
                    Mockito.eq("filter"),
                    Mockito.eq("filter"),
                    Mockito.eq("filter"),
        ...
        Page<UserDTO> users = userService.getAllManagedUsers(
        ...
        Mockito
            .verify(userMapper, Mockito.times(20))
        ...
        Mockito
            .verify(userMapper, Mockito.times(30))
            .toUserDTO(Mockito.any(User.class)); // 10
        // more
    }
    ...
    public void testGetCurrentUser() {
        ...
        Mockito
            .when(
                userRepository.findOneByEmailIgnoreCase(Mockito.eq("username"))
            )
            .thenReturn(Optional.of(impersonatingUser));
        Mockito
            .when(
                userRepository.findOneByEmailIgnoreCase(
                    Mockito.eq("impersonated-user")
                )
            )
            .thenReturn(Optional.of(impersonatedUser));
    ...
    public void testGetAllUsersBySalesforceId() {
        ...
        Mockito
            .when(
                userRepository.findByDeletedIsFalseAndSalesforceIdAndMemberNameContainingIgnoreCaseOrDeletedIsFalseAndSalesforceIdAndFirstNameContainingIgnoreCaseOrDeletedIsFalseAndSalesforceIdAndLastNameContainingIgnoreCaseOrDeletedIsFalseAndSalesforceIdAndEmailContainingIgnoreCase(
                    Mockito.any(Pageable.class),
                    Mockito.anyString(),
                    Mockito.anyString(),
                    Mockito.anyString(),
        ...
        Page<UserDTO> page = userService.getAllUsersBySalesforceId(
            Mockito.mock(Pageable.class),
            "some-salesforce-id"
        );
    ...
    public void testSendActivationReminders() {
        ...
        Mockito
            .when(userRepository.findAllByActivatedIsFalseAndDeletedIsFalse())
            .thenReturn(Arrays.asList(user1, user2, user3, user4, user5));

        userService.sendActivationReminders();

        Mockito
            .verify(userRepository, Mockito.times(4))
            .save(userCaptor.capture());

        List<User> captured = userCaptor.getAllValues();
        assertThat(captured.get(1).getEmail()).isEqualTo("2@user.com");
        assertThat(captured.get(1).getActivationReminders().size())
            .isEqualTo(2);
        assertThat(
            captured.get(1).getActivationReminders().get(0).getDaysElapsed()
        )
            .isEqualTo(7);
        assertThat(
            captured.get(1).getActivationReminders().get(1).getDaysElapsed()
        )
            .isEqualTo(30);
        assertThat(captured.get(3).getEmail()).isEqualTo("3@user.com");
        assertThat(captured.get(3).getActivationReminders().size())
            .isEqualTo(1);
        assertThat(
            captured.get(3).getActivationReminders().get(0).getDaysElapsed()
        )
            .isEqualTo(7);
    }
    ...
    public void testDisableMfa() {
        Mockito
            ...
            .thenReturn(Optional.of(getUserUsingMfa()));
    ...
    public void testValidateOtp() {
        ...
        Assertions.assertThrows(
            MfaAuthenticationFailureException.class,
            () -> {
    ...
    public void testValidMfaCode() {
        ...
        user.setMfaBackupCodes(
            Arrays.asList(backupCode1, backupCode2, backupCode3)
        );

        Mockito
            .when(userRepository.findOneByEmailIgnoreCase(Mockito.eq("user")))
            .thenReturn(Optional.of(user));
        userService.validMfaCode("user", "backupCode2");

        Mockito
            .verify(passwordEncoder, Mockito.times(3))
            .matches(Mockito.eq("backupCode2"), Mockito.anyString());
        Mockito
            .verify(userRepository)
            .findOneByEmailIgnoreCase(Mockito.eq("user"));
        Mockito.verify(userRepository).save(userCaptor.capture());
    ...
    public void testEnableMfa() {
        Mockito
            ...
            .thenReturn(Optional.of(getUser("username")));
        Mockito
            .when(encryptUtil.encrypt(Mockito.anyString()))
            .thenAnswer(
                new Answer<String>() {
                    @Override
        ...
        assertThat(backupCodes.size())
            .isEqualTo(UserService.BACKUP_CODE_BATCH_SIZE);
        ...
        assertThat(captured.getMfaEncryptedSecret())
            .isEqualTo("encrypted-" + secret);
        assertThat(captured.getMfaBackupCodes()).isNotNull();
        assertThat(captured.getMfaBackupCodes().size())
            .isEqualTo(UserService.BACKUP_CODE_BATCH_SIZE);
        captured
            .getMfaBackupCodes()
            .forEach(c -> {
    ...
    void testUpdateUsersSalesforceId() {
        List<User> firstPage = getUsersForSalesforceId(
            "salesforce-id",
            0,
            UserService.BATCH_SIZE
        );
        List<User> secondPage = getUsersForSalesforceId(
            "salesforce-id",
            UserService.BATCH_SIZE,
            UserService.BATCH_SIZE * 2
        );
        List<User> thirdPage = getUsersForSalesforceId(
            "salesforce-id",
            UserService.BATCH_SIZE * 2,
        ...
        Mockito
            ...
            .thenReturn(new PageImpl<User>(secondPage))
            .thenReturn(new PageImpl<User>(thirdPage))
            .thenReturn(new PageImpl<User>(new ArrayList<>()));
        boolean success = userService.updateUsersSalesforceId(
            "salesforce-id",
            "new-salesforce-id"
        );
        ...
        Mockito
            .verify(
                userRepository,
                Mockito.times((UserService.BATCH_SIZE * 3) - 10)
            )
            .save(userCaptor.capture());

        List<User> saved = userCaptor.getAllValues();
        saved.forEach(u ->
            assertThat(u.getSalesforceId()).isEqualTo("new-salesforce-id")
        );
    }
    ...
    void testUpdateUsersSalesforceIdWithFailure() {
        ...
        Mockito
            .verify(userRepository, Mockito.times(7))
            .save(userCaptor.capture());
        List<User> saved = userCaptor.getAllValues();
    ...
    private List<User> getUsersForSalesforceId(
        String salesforceId,
        int from,
        int to
    ) {
